...
# positionプロパティ
static
特徴: これがデフォルトの値です。要素は通常のフローに従って配置されます。top、right、bottom、left プロパティは影響しません。
使い方: 特に位置を変更したいというニーズがない場合は、この値が適用されます。

relative
特徴: 要素は通常のフローに従って配置されますが、top、right、bottom、left プロパティでのオフセットが適用されるようになります。
使い方: 要素を少し移動させたい、または絶対位置 (absolute) の子要素を持つ要素の基準点として使用したい場合に適しています。

absolute
特徴: 要素は通常のドキュメントのフローから取り除かれ、最も近い position が relative、absolute、fixed、または sticky の親要素に対して配置されます。もしそのような親要素がない場合は、ページの<html> 要素に対して配置されます。
使い方: 要素を完全に自由に配置したい場合や、親要素の特定の位置に要素を配置したい場合に使用します。

fixed
特徴: 要素はビューポート（ブラウザの表示領域）に対して配置されます。スクロールしても、要素は固定された位置にとどまります。
使い方: スクロール時にも常に表示されるヘッダーやフッター、バックトップボタンなどを作成する際に使用します。

sticky
特徴: この値は relative と fixed の組み合わせのように動作します。要素は特定のスクロール位置まで relative として動作し、その位置を超えると fixed のように動作します。
使い方: スクロール時に一定の位置で要素を固定したい場合、例えば "sticky" ヘッダーやセクションのタイトルなどに使用します。
